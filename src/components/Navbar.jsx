            ...
            <div className={`flex justify-center min-w-full ${dark ? 'bg-[#000000]' : 'bg-[#FAFAFC]'} container`}>
                <div className={`font-semibold w-3/4 min-h-16 ${dark ? 'text-[#FAFAFC]' : 'text-[#1F1F1F]'} flex justify-evenly items-center`}>
                    ...
                    <div onClick={() => setShowDropdown(!showDropdown)} className="relative" style={linkStyle}>
                        ...
                        {showDropdown && (
                            <div className={`absolute w-48 py-2 mt-2  border bg-[#000000] rounded-md shadow-xl top-full ${dark ? ' bg-[#000000]' : ' bg-[#FAFAFC]'}`}>
                                <Link to={"/perniagaan"} className={`block px-4 py-2 text-sm ${dark ? 'text-[#FAFAFC] bg-[#000000] hover:bg-[#121212]' : 'text-[#1F1F1F] bg-[#FAFAFC] hover:bg-[#e8e8e8]'}`} style={linkStyle}>Zakat Perniagaan</Link>
                                <Link to={"/pendapatan"} className={`block px-4 py-2 text-sm ${dark ? 'text-[#FAFAFC] bg-[#000000] hover:bg-[#121212]' : 'text-[#1F1F1F] bg-[#FAFAFC] hover:bg-[#e8e8e8]'}`} style={linkStyle}>Zakat Pendapatan</Link>
                                <Link to={"/emas"} className={`block px-4 py-2 text-sm ${dark ? 'text-[#FAFAFC] bg-[#000000] hover:bg-[#121212]' : 'text-[#1F1F1F] bg-[#FAFAFC] hover:bg-[#e8e8e8]'}`} style={linkStyle}>Zakat Emas</Link>
                                <Link to={"/wangSimpanan"} className={`block px-4 py-2 text-sm ${dark ? 'text-[#FAFAFC] bg-[#000000] hover:bg-[#121212]' : 'text-[#1F1F1F] bg-[#FAFAFC] hover:bg-[#e8e8e8]'}`} style={linkStyle}>Zakat Wang Simpanan</Link>
                            </div>
                        )}
                    </div>
